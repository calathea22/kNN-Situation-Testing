...
import numpy as np
import os
from optimize_euclidean_distances import optimize_weighted_euclidean
from optimize_mahalanobis_distances import optimize_mahalanobis
...
def perform_optimization(location, n, n_biased, test_percentage, val_percentage):
    ...
    train_data_dict, test_data_dict, val_data_dict = simulate_non_explainable_discrimination(n, n_biased, test_percentage, val_percentage)
    ...
    train_disc_free_labels = train_data_dict['disc_free_labels']
    print(train_disc_free_labels)
    ...
    # possible_lambdas = np.arange(0.05, 0.4, 0.05)
    ...
    #     mahalanobis_matrix = optimize_mahalanobis(standardized_train_data, train_class_label, train_protected_info, indices_info, 1, 2, lambda_l1_norm)
    #     print(mahalanobis_matrix)
    #
    #     utils.store_in_excel(weights_euclidean,
    #                          parent_path + location+"/lambda = " +format(lambda_l1_norm, ".2f"), "/euclidean_weights.xlsx")
    #     utils.store_in_excel(mahalanobis_matrix,
    #                          parent_path + location + "/lambda = " +format(lambda_l1_norm, ".2f"), "/mahalanobis_matrix.xlsx")
    ...
    utils.store_in_excel(train_discriminated_instances, train_path, "/discriminated_instances.xlsx")
    utils.store_in_excel(train_disc_free_labels, train_path, "/disc_free_labels.xlsx")
